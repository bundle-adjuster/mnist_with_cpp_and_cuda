...
void cuda_matrix_multiply(const float *A, const float *B, float *C, int M,
                          int N, int K) {
...
// Add bias kernel
__global__ void add_bias_kernel(float *output, const float *bias, int rows,
                                int cols) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  int total = rows * cols;

  if (idx < total) {
    int col = idx % cols; // neuron index
    output[idx] += bias[col];
  }
}

void cuda_add_bias(float *output, const float *bias, int rows, int cols) {
  int total = rows * cols;
...
__global__ void compute_output_delta_kernel(const float *output,
                                            const float *target_onehot,
...
void cuda_compute_output_delta(const float *output, const float *target_onehot,
...
// next_weights connects current layer to next layer, stored as (current_size x
// next_size) For backpropagation: delta[current] = (next_weights^T *
// delta[next]) .* relu_derivative(activations[current]) Since weights[neuron *
// next_size + j] is the weight from neuron to j, we compute:
// delta[current][neuron] = sum_j (weights[neuron * next_size + j] *
// delta[next][j]) * relu_derivative
__global__ void compute_hidden_delta_kernel(const float *next_delta,
                                            const float *next_weights,
                                            const float *activations,
                                            float *delta, int current_size,
                                            int next_size, int batch_size) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if (idx < total) {
    ...
    for (int j = 0; j < next_size; j++) {
      sum += next_delta[batch * next_size + j] *
             next_weights[neuron * next_size + j];
    }

    // Apply Leaky ReLU derivative: if activation <= 0, gradient is alpha
    // (0.01), else 1.0
...
void cuda_compute_hidden_delta(const float *next_delta,
                               const float *next_weights,
  ...
  compute_hidden_delta_kernel<<<numBlocks, BLOCK_SIZE>>>(
      next_delta, next_weights, activations, delta, current_size, next_size,
      batch_size);
  CUDA_CHECK(cudaDeviceSynchronize());
}

// Update weights
// weights is stored as (input_size x output_size), where weights[input_idx *
// output_size + output_idx] Gradient: dW = (1/batch) * prev_activations^T *
// delta prev_activations is (batch_size x input_size), delta is (batch_size x
// output_size) So dW is (input_size x output_size)
__global__ void update_weights_kernel(float *weights, const float *delta,
  ...
  if (input_idx < input_size && output_idx < output_size) {
    ...
    for (int b = 0; b < batch_size; b++) {
      grad += prev_activations[b * input_size + input_idx] *
              delta[b * output_size + output_idx];
    }
...
void cuda_update_weights(float *weights, const float *delta,
                         const float *prev_activations, float learning_rate,
  ...
  update_weights_kernel<<<gridSize, blockSize>>>(
      weights, delta, prev_activations, learning_rate, input_size, output_size,
      batch_size);
  CUDA_CHECK(cudaDeviceSynchronize());
}

// Update biases
__global__ void update_biases_kernel(float *biases, const float *delta,
...
void cuda_update_biases(float *biases, const float *delta, float learning_rate,
                        int size, int batch_size) {
  int numBlocks = (size + BLOCK_SIZE - 1) / BLOCK_SIZE;
  update_biases_kernel<<<numBlocks, BLOCK_SIZE>>>(biases, delta, learning_rate,
                                                  size, batch_size);
  CUDA_CHECK(cudaDeviceSynchronize());
}

// ReLU derivative
__global__ void relu_derivative_kernel(const float *activations, float *delta,
                                       int size) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < size) {
...
void cuda_relu_derivative(const float *activations, float *delta, int size) {
  int numBlocks = (size + BLOCK_SIZE - 1) / BLOCK_SIZE;
...
__global__ void leaky_relu_derivative_kernel(const float *pre_activations,
                                             float *delta, int size,
                                             float alpha) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < size) {
...
void cuda_leaky_relu_derivative(const float *pre_activations, float *delta,
                                int size, float alpha) {
  int numBlocks = (size + BLOCK_SIZE - 1) / BLOCK_SIZE;
  leaky_relu_derivative_kernel<<<numBlocks, BLOCK_SIZE>>>(pre_activations,
                                                          delta, size, alpha);
  CUDA_CHECK(cudaDeviceSynchronize());
}

// Cross-entropy loss
__global__ void cross_entropy_loss_kernel(const float *output,
                                          const int *target, float *loss,
...
void cuda_cross_entropy_loss(const float *output, const int *target,
                             float *loss, int num_classes, int batch_size) {
  int numBlocks = (batch_size + BLOCK_SIZE - 1) / BLOCK_SIZE;
  cross_entropy_loss_kernel<<<numBlocks, BLOCK_SIZE>>>(output, target, loss,
